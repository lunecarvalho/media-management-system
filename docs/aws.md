...
7. Configurar /health/ como health check de liveness do Beanstalk: GET/HEAD público, sem consulta ao banco, HTTP 200 e JSON `{"status":"ok"}` enquanto o Django responde. Não mede disponibilidade do RDS. Para monitorar dependências, /readiness/ executa SELECT 1 e retorna HTTP 200 com status ok ou HTTP 503 com status unavailable, sem detalhes do erro. Readiness permanece sob HTTPS e não exige autenticação; configure seu monitor separadamente. Ambas as respostas proíbem cache e os endpoints não aceitam alterações via POST. Configure a sondagem/proxy com Host aceito pelo Django: ALLOWED_HOSTS continua sendo verificado. Não resolver health check com wildcard de hosts.
